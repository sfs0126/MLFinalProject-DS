...
"""Likewise we use SafeDataLoader to load in our batch size for training the model (about half the dimensions of our images), and then we load in our validation set as well."""
...
"""We can double check to see if the original image dimensions were reduced to just a 10 input vector like the last argument of our fully connected layer intended, and it worked, since we went from an image with dimensions 3x128x128 and a batch size of 64 to just a single vector of 10 probabilities with the same batch size."""
